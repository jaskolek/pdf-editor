    ...
    public function resize($source, $width, $height, $newWidth, $newHeight): string
    {
        ...
        for ($channelIndex = 0; $channelIndex < 4; ++$channelIndex) {
            ...
            for ($pixelIndex = 0; $pixelIndex < $length; ++$pixelIndex) {
                $pixelValue = \ord($source[$pixelIndex * 4 + $channelIndex]);
                $color = imagecolorallocate($channelImage, $pixelValue, $pixelValue, $pixelValue);
                $x = $pixelIndex % $width;
            ...
            imagedestroy($channelImage);

            for ($y = 0; $y < $newHeight; ++$y) {
                for ($x = 0; $x < $newWidth; ++$x) {
                    $pixelIndex = ($x + $y * $newWidth) * 4 + $channelIndex;
                    $imageColor = imagecolorat($resizedChannelImage, $x, $y);
                    $colorValue = imagecolorsforindex($resizedChannelImage, $imageColor);
                    $pixelValue = \chr(floor($colorValue['red'] / $this->channelColorReductionLevel) * $this->channelColorReductionLevel);
//                    $pixelValue = \chr($colorValue['red']);

                    $outputCharacterList[$pixelIndex] = $pixelValue;
                }
            }
            imagedestroy($resizedChannelImage);
        }
        ksort($outputCharacterList);
        return implode('', $outputCharacterList);
    }
